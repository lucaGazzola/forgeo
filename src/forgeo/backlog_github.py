    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise GithubRequestError(f"{method} {request.full_url} failed: {exc}") from exc
        if not raw.strip():
            return {}
        try:
            data = json.loads(raw)
...
class GithubBacklog(IssueBacklogBase):
    ...
    async def _get_issue(self, issue_id: str) -> dict[str, Any] | None:
        try:
            ...
            # GitHub issue ids are numeric; but allow string like "123"
            # Try to parse as int after stripping prefix? tests use numeric strings
            try:
    ...
    def _comment(self, issue_number: int, state: str, reason: list[str]) -> None:
        ...
        self._pending_comments.append((issue_number, f"[forgeo] {state}\n{text}"))
